...
public class JdDetailFragment extends BaseFragment<JanDanPresenter> implements JanDanContract.View {
    ...
    public void bindView(View view, Bundle savedInstanceState) {
        ...
        mAdAapter.setEnableLoadMore(true);
        mAdAapter.setPreLoadNumber(1);
        mAdAapter.setLoadMoreView(new CustomLoadMoreView());
        mAdAapter.openLoadAnimation(BaseQuickAdapter.ALPHAIN);
        mAdAapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            @Override
    ...
    public void loadFreshNews(FreshNewsBean freshNewsBean) {
        if (freshNewsBean == null) {
            ...
            pageNum++;
            mAdAapter.setNewData(freshNewsBean.getPosts());
            mPtrFrameLayout.refreshComplete();
            showSuccess();
        }
    }
    ...
    public void loadDetailData(String type, JdDetailBean jdDetailBean) {
        if (jdDetailBean == null) {
            ...
            pageNum++;
            mAdAapter.setNewData(jdDetailBean.getComments());
            mPtrFrameLayout.refreshComplete();
            showSuccess();
        }
    }
    ...
    public void loadMoreFreshNews(FreshNewsBean freshNewsBean) {
        if (freshNewsBean == null) {
            mAdAapter.loadMoreFail();
        } else {
            pageNum++;
            mAdAapter.addData(freshNewsBean.getPosts());
            mAdAapter.loadMoreComplete();
        }
    }
    ...
    public void loadMoreDetailData(String type, JdDetailBean jdDetailBean) {
        if (jdDetailBean == null) {
            ...
            pageNum++;
            mAdAapter.addData(jdDetailBean.getComments());
            mAdAapter.loadMoreComplete();
        }
    }
}
